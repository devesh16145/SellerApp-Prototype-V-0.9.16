...
export default function RecentOrders() {
    ...
    const fetchOrders = async () => {
      ...
      try {
        const { data, error } = await supabase
          ...
          .limit(4); // Limit to recent orders, adjust limit as needed

        if (error) {
          console.error('Error fetching orders:', error);
        } else {
          console.log("Data from Supabase (RecentOrders):", data); // ADDED CONSOLE LOG
          setOrders(data);
        }
      } finally {
        setLoading(false);
  ...
  if (loading) {
    return <div>Loading Recent Orders...</div>; // Basic loading state
  }
            ...
            {(order.status === "New" || order.status === "Pending") && (
              <div className="absolute top-0 left-0 w-full h-8 bg-red-600 flex items-center justify-between px-3 rounded-t-lg">
                ...
                <div className='flex items-center gap-2'>
                  {console.log("timeLeft for recent order:", order.order_number, order.timeLeft)} {/* ADDED LOG */}
