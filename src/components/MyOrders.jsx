...
export default function MyOrders() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      setLoading(true);
      try {
        // Modified Supabase query to fetch order items and product names
        ...
        if (error) {
          ...
          console.log("Data from Supabase (MyOrders):", data); // ADDED CONSOLE LOG
          setOrders(data);
        }
      } finally {
        setLoading(false);
  ...
  const filteredOrders = orders.filter(order => {
    if (!order || !order.order_items) return false; // Check for order_items existence

    const query = searchQuery.toLowerCase();
    // Access product name from the nested products table
    const itemMatch = order.order_items.some(item =>
      item.products?.name?.toLowerCase().includes(query)
    );
    const orderIdMatch = order.order_number.toLowerCase().includes(query); // Use order_number from Supabase
  ...
  if (loading) {
    return <div>Loading Orders...</div>; // Basic loading state
  }
          ...
          <div key={order.id} className="bg-agri-gray rounded-lg relative">
            {(order.status === "New" || order.status === "Pending") && (
              <div className="absolute top-0 left-0 w-full h-8 bg-red-600 flex items-center justify-between px-3 rounded-t-lg">
                ...
                <div className='flex items-center gap-2'>
                  {console.log("timeLeft for order:", order.order_number, order.timeLeft)} {/* ADDED LOG */}
            ...
            <div className="p-3 pt-10 space-y-2">
              <div className="flex justify-between items-center">
                <div>
                  <div className="font-semibold text-sm">{order.order_number}</div> {/* Use order_number from Supabase data */}
              ...
              <div className="space-y-1">
                {/* Access order_items from the fetched data */}
              ...
              <div className="pt-2 border-t border-gray-200 flex items-center justify-between">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xs font-semibold">₹{order.total_amount}</span> {/* Use total_amount from Supabase data */}
                </div>
